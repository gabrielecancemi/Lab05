...
class View(ft.UserControl):
    ...
    def load_interface(self):
        ...
        self.btnCercaIscritti = ft.ElevatedButton(
            text="Cerca iscritti",
            on_click=self._controller.cercaIscritti
        )

        row1 = ft.Row([self.ddInCorso, self.btnCercaIscritti],
                      alignment=ft.MainAxisAlignment.CENTER)
        self._page.controls.append(row1)

        self.txtInMatricola = ft.TextField(
            label="matricola",
            width=200
        )
        ...
        self.btnCercaStudente = ft.ElevatedButton(
            text="Cerca studente",
            on_click=self._controller.cercaStudente
        )

        self.btnCercaCorsi = ft.ElevatedButton(
            text="Cerca corsi",
            on_click=self._controller.cercaCorsi
        )

        self.btnIscrivi = ft.ElevatedButton(
            text="Iscrivi",
